...
import com.smi.generated.crs_sm_tnd.*;
...
@Service
public class CrsSmTndService {

    private static final String CODE_ANNEXE = "CRS-SMTND";
    ...
    private Document.Extraits.Extrait buildExtraitFromRow(ObjectFactory factory, Row row) {
        ...
        titulaire.setRaisSociale(getStringValue(row.getCell(3))); // D: RaisSociale
        titulaire.setNationalite(getStringValue(row.getCell(4))); // E: Nationalite
        entete.setTitulaire(titulaire);

        // Référence Compte (Colonnes F à N)
        Document.Extraits.Extrait.Entete.RefCompte refCompte = factory.createDocumentExtraitsExtraitEnteteRefCompte();
        refCompte.setRib(getStringValue(row.getCell(5))); // F: Rib
        refCompte.setDateOuvCpte(getStringValue(row.getCell(6))); // G: DateOuvCpte

        if (row.getCell(7) != null) refCompte.setNumAutBCT(getStringValue(row.getCell(7))); // H: NumAutBCT (optionnel)
        if (row.getCell(8) != null) refCompte.setDateAutBCT(getStringValue(row.getCell(8))); // I: DateAutBCT (optionnel)

        refCompte.setEtatCpte(BigInteger.valueOf(new BigDecimal(getStringValue(row.getCell(9))).intValue())); // J: EtatCpte
        if (row.getCell(10) != null) refCompte.setDateclotureCpte(getStringValue(row.getCell(10))); // K: DateclotureCpte (optionnel)
        ...
        if (row.getCell(16) != null) {
            ...
            refOperation.setDateMvt(getStringValue(row.getCell(22))); // W: DateMvt

            if (row.getCell(23) != null) refOperation.setCodMvt(getStringValue(row.getCell(23))); // X: CodMvt (optionnel)
            if (row.getCell(24) != null) refOperation.setNatOp(getStringValue(row.getCell(24))); // Y: NatOp (optionnel)
            if (row.getCell(25) != null) refOperation.setModReg(String.valueOf(new BigDecimal(getStringValue(row.getCell(25))).intValue())); // Z: ModReg (optionnel)
            if (row.getCell(26) != null) refOperation.setNumMsgeSwiftMvt(getStringValue(row.getCell(26))); // AA: NumMsgeSwiftMvt (optionnel)
            ...
            if (row.getCell(27) != null) refAuth.setNumAutBCT(getStringValue(row.getCell(27))); // AB: NumAutBCT (optionnel)
            if (row.getCell(28) != null) refAuth.setDateAutBCT(getStringValue(row.getCell(28))); // AC: DateAutBCT (optionnel)
            detail.setRefAutorisationBct(refAuth);

            detail.setDenomBenif(getStringValue(row.getCell(29))); // AD: DenomBenif
            if (row.getCell(30) != null) detail.setPays(getStringValue(row.getCell(30))); // AE: Pays (optionnel)
