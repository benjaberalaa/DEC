...
import com.smi.generated.ds_ietr.*;
...
@Service
public class DsIetrService {

    private static final String CODE_ANNEXE = "DS-IETR";
    ...
    public Document processExcelFile(MultipartFile file) throws IOException {
        try (InputStream is = file.getInputStream();
            ...
            while (rowIterator.hasNext()) {
                ...
                detail.setFormInvt(getStringValue(dataRow.getCell(4))); // Colonne E: FormInvt
                ...
                documentInv.setCodDoc(getStringValue(dataRow.getCell(5))); // Colonne F: CodDoc
                documentInv.setNomOrigPiece(getStringValue(dataRow.getCell(6))); // Colonne G: NomOrigPiece
                documentInv.setAttachefFile(getStringValue(dataRow.getCell(7)).getBytes()); // Colonne H: AttachefFile
            ...
            dossiers.setDossier(dossier); // Notez que c'est setDossier() et non add()
    ...
    private Document.Dossiers.Dossier.Details.Detail buildDetailFromRow(ObjectFactory factory, Row row) {
        Document.Dossiers.Dossier.Details.Detail detail = factory.createDocumentDossiersDossierDetailsDetail();

        // Informations de base
        detail.setAgence(getStringValue(row.getCell(1))); // Colonne B: Agence
        detail.setMatFiscal(getStringValue(row.getCell(2))); // Colonne C: MatFiscal
        detail.setRaisSoc(getStringValue(row.getCell(3))); // Colonne D: RaisSoc
        detail.setFormInvt(getStringValue(row.getCell(4))); // Colonne E: FormInvt

        // Documents
        Document.Dossiers.Dossier.Details.Detail.Documents documents = factory.createDocumentDossiersDossierDetailsDetailDocuments();
        Document.Dossiers.Dossier.Details.Detail.Documents.DocumentInv documentInv = factory.createDocumentDossiersDossierDetailsDetailDocumentsDocumentInv();

        documentInv.setCodDoc(getStringValue(row.getCell(5))); // Colonne F: CodDoc
        documentInv.setNomOrigPiece(getStringValue(row.getCell(6))); // Colonne G: NomOrigPiece
        documentInv.setAttachefFile(getStringValue(row.getCell(7)).getBytes()); // Colonne H: AttachefFile (base64)
