    ...
    function traverse(node, currentPath) {
        ...
        if (elements.length > 0) {
            elements.forEach(el => {
                ...
                if (['CodeIAT', 'DateDec', 'CodeAnnexe', 'PeriodDec', 'AnneDec'].includes(name) && !currentPath.includes('Entete') && !currentPath.includes('Detail')) return;
...
function processDecl(declKey) {
    ...
    parser.parseString(xml, (err, result) => {
        let fields = parseXsdSchema(result).filter(f => f.includes('Entete') || f.includes('Detail') || f.includes('Agence') || f.includes('NbrEcritures'))
        ...
        if (excelFile) {
            try {
                ...
                for (let i = 0; i < fields.length; i++) ws[xlsx.utils.encode_cell({c: i, r: 0})] = { t: 's', v: fields[i] };
                let range = xlsx.utils.decode_range(ws['!ref'] || 'A1:A1');
                if (range.e.c < fields.length - 1) { range.e.c = fields.length - 1; if(range.e.r === 0) range.e.r = 1; ws['!ref'] = xlsx.utils.encode_range(range); }
        ...
        let fieldsStr = fields.map(k => {
            ...
            if(label.includes('Date')) type = 'date'; if(label === 'Value') type = 'number';
            return `      {\n            "id": "${k}",\n            "name": "${k}",\n            "label": "${label}",\n            "type": "${type}"\n      }`;
        }).join(',\n');
